...
class Database:
    ...
    def add_weekly_ranking(self, team_id, year, week, offense, defense):
        try:
            logger.info(
                f"Adding new ranking year={year} week={week} team_id={team_id} offense={offense} defense={defense}"
            )
            self.cur.execute(
                "INSERT INTO weekly_rankings (team_id,year,week,offense_rank,defense_rank) VALUES (?,?,?,?,?)",
                (team_id, year, week, offense, defense),
            )
        except sqlite3.Error as error:
            logger.error(
